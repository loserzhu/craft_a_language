...
import {SymKind, FunctionSymbol, VarSymbol, built_ins} from './symbol';
...
class Enter extends SemanticAstVisitor {
	...
	visitFunctionDecl(functionDecl: FunctionDecl): any {
		const currentScope = this.scope as Scope;

		//创建函数的symbol
		const paramTypes: Type[] = [];
		if (functionDecl.callSignature.paramList !== null) {
			for (const p of functionDecl.callSignature.paramList.params) {
		...
		if (currentScope.hasSymbol(functionDecl.name)) {
			this.addError(
				'Duplicate symbol: ' + functionDecl.name,
				functionDecl
			);
		} else {
		...
		//修改当前的函数符号
		const lastFunctionSym = this.functionSym;
		this.functionSym = sym;
	...
	visitVariableDecl(variableDecl: VariableDecl): any {
		...
		if (currentScope.hasSymbol(variableDecl.name)) {
			this.addError(
				'Duplicate symbol: ' + variableDecl.name,
				variableDecl
			);
		}
		//把变量加入当前的符号表
		const sym = new VarSymbol(variableDecl.name, variableDecl.theType);
...
class RefResolver extends SemanticAstVisitor {
	...
	visitVariableDecl(variableDecl: VariableDecl): any {
		const currentScope = this.scope as Scope;
		const declaredSyms = this.declaredVarsMap.get(currentScope) as Map<
			string,
			VarSymbol
		>;
		const sym = currentScope.getSymbol(variableDecl.name);
		if (sym !== null) {
			//TODO 需要检查sym是否是变量
	...
	private findVariableCascade(
		...
		const declaredSyms = this.declaredVarsMap.get(scope) as Map<
			string,
			VarSymbol
		>;
		const symInScope = scope.getSymbol(variable.name);
		if (symInScope !== null) {
...
class LeftValueAttributor extends SemanticAstVisitor {
	...
	visitFunctionCall(functionCall: FunctionCall): any {
		if (this.parentOperator === Op.Dot) {
			const functionType = functionCall.theType as FunctionType;
...
class TypeChecker extends SemanticAstVisitor {
	visitVariableDecl(variableDecl: VariableDecl): any {
		...
		if (variableDecl.init !== null) {
			const t1 = variableDecl.theType as Type;
			const t2 = variableDecl.init.theType as Type;
			if (!t2.LE(t1)) {
				this.addError(
	...
	visitBinary(bi: Binary): any {
		...
		const t1 = bi.exp1.theType as Type;
		const t2 = bi.exp2.theType as Type;
	...
	visitFunctionCall(functionCall: FunctionCall): any {
		if (functionCall.sym !== null) {
			const functionType = functionCall.sym.theType as FunctionType;
			...
			for (let i = 0; i < functionCall.arguments.length; i++) {
				...
				if (i < functionType.paramTypes.length) {
					const t1 = functionCall.arguments[i].theType as Type;
					const t2 = functionType.paramTypes[i] as Type;
 ...
 * 常量折叠
 */
export class ConstFolder extends SemanticAstVisitor {
	visitBinary(bi: Binary): any {
		const v1 = bi.exp1.constValue;
		const v2 = bi.exp2.constValue;
		if (Operators.isAssignOp(bi.op)) {
