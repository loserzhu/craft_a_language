...
export class SimpleType extends Type {
	...
	hasVoid(): boolean {
		if (this === SysTypes.Void) {
			...
			for (const t of this.upperTypes) {
				if (t.hasVoid()) {
					return true;
				}
			}
	...
	toString(): string {
		let upperTypeNames = '[';
		for (const ut of this.upperTypes) {
	...
	LE(type2: Type): boolean {
		if (type2 === SysTypes.Any) {
			...
			const t = type2 as UnionType;
			if (t.types.indexOf(this) !== -1) {
				return true;
			} else {
				//是联合类型中其中一个类型的子类型就行
				for (const t2 of t.types) {
					if (this.LE(t2)) {
						return true;
					}
				}
...
export class FunctionType extends Type {
	...
	toString(): string {
		let paramTypeNames = '[';
		for (const ut of this.paramTypes) {
...
export class UnionType extends Type {
	...
	static index = 0; //序号，用于给UnionType命名
	...
	LE(type2: Type): boolean {
		if (type2 === SysTypes.Any) {
			...
			for (const t1 of this.types) {
				let found = false;
				for (const t2 of (type2 as UnionType).types) {
					if (t1.LE(t2)) {
						found = true;
						break;
					}
